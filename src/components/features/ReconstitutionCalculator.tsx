...
/** Reconstitution figures the caller can copy into a form. */
export interface AppliedReconstitution {
  vialQuantity: number;
  vialUnit: string;
  bacWaterMl: number;
...
function appliedFrom(outcome: Outcome): AppliedReconstitution {
  return {
    vialQuantity: outcome.inputs.vialQuantity,
    vialUnit: outcome.inputs.vialUnit,
    bacWaterMl: roundVolume(outcome.result.volumeMl),
    concentrationMgPerMl: outcome.result.concentrationMgPerMl,
    syringeType: outcome.inputs.syringeType,
  };
}

/**
 * The reconstitution calculator (BAC water) — the interactive form + result
 * block shared by the Calculator page and the in-treatment reconstitute dialog.
 * `onCalculated` fires after each successful calculation (the page uses it to
 * save history); when `onApply` is set, an "apply" button lets the caller copy
 * the computed vial figures into its own form.
 */
export function ReconstitutionCalculator({
  ...
  onCalculated?: (outcome: Outcome) => void | Promise<void>;
  onApply?: (values: AppliedReconstitution) => void;
  applyLabel?: string;
  outcomeFooter?: ReactNode;
}) {
  const { t } = useI18n();
  ...
  const settle = async (next: Outcome) => {
    setOutcome(next);
    setApplied(false);
    if (onCalculated) await onCalculated(next);
  };
  ...
  const onSubmitConcentration = async (values: CalculatorInput) => {
    ...
    try {
      ...
      await settle({ mode: "concentration", result, inputs: values });
    } catch (error) {
      handleError(error);
    }
  };
  ...
  const onSubmitUnits = async (values: SyringeCalculatorInput) => {
    ...
    try {
      ...
      await settle({ mode: "units", result, inputs: values });
    } catch (error) {
      handleError(error);
    }
  };
  ...
  const onSubmitWeight = async (values: WeightCalculatorInput) => {
    ...
    try {
      ...
      await settle({ mode: "weight", result, inputs: values });
    } catch (error) {
      handleError(error);
    }
  };
  ...
  const onSubmitVolume = async (values: VolumeCalculatorInput) => {
    ...
    try {
      ...
      await settle({ mode: "volume", result, inputs: values });
    } catch (error) {
      handleError(error);
    }
  };
  ...
  const handleApply = () => {
    if (!outcome || !onApply) return;
    onApply(appliedFrom(outcome));
    setApplied(true);
  };
      ...
      {mode === "concentration" ? (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              ...
              placeholder="2"
              error={
                concentrationForm.formState.errors.desiredConcentration?.message
              }
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <Select label={t("calc.unit")} {...unitsForm.register("vialUnit")}>
              <option value="mg">mg</option>
              <option value="mcg">mcg</option>
            </Select>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <Select label={t("calc.unit")} {...weightForm.register("vialUnit")}>
              <option value="mg">mg</option>
              <option value="mcg">mcg</option>
            </Select>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <Select label={t("calc.unit")} {...volumeForm.register("vialUnit")}>
              <option value="mg">mg</option>
              <option value="mcg">mcg</option>
            </Select>
          </div>
        ...
        <div className="mt-5 rounded-2xl bg-tan-faint border border-tan-soft p-5">
          ...
          {outcome.mode === "weight" || outcome.mode === "volume" ? (
            <>
              <p className="mt-2 text-3xl font-semibold tracking-tight text-ink">
                {formatAmount(roundVolume(outcome.result.concentrationMgPerMl))}{" "}
                mg/mL
              </p>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-3 text-sm">
                <Stat
                  ...
                  value={`${formatAmount(roundVolume(outcome.result.volumeMl))} mL`}
                />
                <Stat
            ...
            <>
              ...
              <p className="mt-1 text-sm text-muted">{t("calc.resultSuffix")}</p>
            </>
          )}
          ...
          {onApply && (
            ...
            >
              ...
              {applied ? t("recon.applied") : applyLabel ?? t("recon.applyCalc")}
            </Button>
          )}
        </div>
      )}
